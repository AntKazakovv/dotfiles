...
module.exports = function changeLogsTask() {
    task('change-logs', async (done) => {
        ...
        if (fs.existsSync('.gitlab-key')) {
            ...
            throw Error('Gitlab api key not found. See https://vcsxa.egamings.com/wlc/devtools/-/blob/master/theme/change-logs.md');
        }
        ...
        for (const item of history) {
            ...
            if (_.includes(commit.title, 'SCR #')) {
                ...
                const response = this.execNativeShellSync(`curl "https://vcsxa.egamings.com/api/v4/projects/315/merge_requests?state=merged&search=${searchText}&private_token=${apiKey}"`);
                ...
                const response = this.execNativeShellSync(`curl "https://vcsxa.egamings.com/api/v4/projects/315/merge_requests?state=merged&source_branch=${sourceBranch}&private_token=${apiKey}"`);
                const data = JSON.parse(response);
                if (_.isArray(data)) {
