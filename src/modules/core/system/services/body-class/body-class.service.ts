...
export type TBodyClassPrefix = 'lang' | 'auth' | 'theme' | 'device';
...
export class BodyClassService {
     ...
     */
    public addModifier(modifier: string, skipEvent?: boolean): void {
        this.document.body.classList.add(modifier);
        if (skipEvent) {
            this.emitClassEvent(modifier, 'add');
        }
    }
    ...
    private initListeners(): void {
        this.eventService.subscribe<IData>([
            {name: 'LOGIN'},
            {name: 'LOGOUT'},
        ], (event) => {
            this.replaceModifier(
                'auth',
                event.name === 'userLogin' ? '1' : '0',
            );
        });
    ...
    private emitClassEvent(className: string, action: 'add' | 'remove'): void {
