        ...
        if (this.$params.valueType === 'realityChecker') {
            ...
            if (value >= 60) {
                this.timeText = Math.floor(value / 60) + ' '
                    + this.translateService.instant(Math.floor(value / 60) < 2 ? gettext('Hour') : gettext('Hours'));
                if (value % 60 !== 0) {
                    this.timeText += ' ' + value % 60 + ' ' + this.translateService.instant('Minutes');
                }
            } else {
                this.timeText = value + ' ' + this.translateService.instant('Minutes');
            }
        }
    }
}
